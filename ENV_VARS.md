...
## Per-user Google Workspace APIs

| Variable | Required | Default | Description |
|---|---|---|---|
| `GOOGLE_API_CLIENT_ID` | for Google Workspace integrations | — | Client ID from the existing **Full Stack** Web OAuth client |
| `GOOGLE_API_CLIENT_SECRET` | for Google Workspace integrations | — | Client secret from that Web OAuth client; never sent to custom MCP URLs |

The OAuth callback obtains each user's access and refresh tokens and stores
