...
### 3.2 MCP is three architectures, not one

The blueprint's MCP policy assumes container MCP services. Reality:
...
3. **Google direct-REST toolsets that only masquerade as MCP** — `build_toolsets_for_user` (`app/services/mcp_connection.py:463-482`) routes recognized Google URLs to local `FunctionToolset`s built from a data registry (`app/agents/google_apis/products.py`, `google_workspace_api.py`); "MCP connection" is the storage/UI abstraction, the transport is plain REST. This migration is in flight — compat env aliases (`GOOGLE_WORKSPACE_MCP_*`, `GOOGLE_DRIVE_*`) still exist in `config.py:140-145`.

Two security-model facts the plan must encode explicitly:

- **Tool approval is asymmetric.** Human-in-the-loop gating uses pydantic-ai deferred tools (`agent_session.py:304-378` pauses the run, emits `tool_approval_required` over WebSocket). It covers **Google mutation tools only** (`products.py:269-276`, Gmail send/draft/delete in `google_workspace_api.py:344-346`). **MCP-sourced tools are never approval-gated.** The blueprint's "write-capable tools require explicit human authorization policy" is currently true for Google and false for arbitrary user-added MCP servers — this is precisely the kind of declared-vs-observed gap the governance system exists to surface, so state it as a known finding, not an assumption.
...
- **I2 — Wrong MCP threat model (high).** Policies written for container MCPs miss the actual privileged surface: per-user DB-resident connections, OAuth-discovered endpoints, the Google REST bypass, and the approval-gating asymmetry (§3.2). The agent/tool/permission graph as drafted would show `github-mcp` (already triple-locked) and be blind to arbitrary user-added write-capable servers.
- **I3 — Boundary model can't express real components (high).** Directory-scoped `.governance.json` cannot describe: `github-mcp` (no directory), the chat feature (spans 4 directory roots), Docker Model Runner (host runtime), or per-user MCP connections (DB rows). Implementing as-written forces fake directories or silent boundary gaps.
...
- **I6 — No enforcement substrate for PR CI (medium).** There is no git remote; the plan's pull-request gates (Phase 5 items 5, 10) have nothing to attach to. Separately, local-only history undermines the plan's own goal of durable change records — a governance system whose entire history sits on one disk.
...
- **I10 — Destructive validation commands (medium).** `.governance.json` `validation` lists could name suites that send real email / mutate Google accounts (`tests/live/`, gated only by env vars). An agent following "run required validation" can be induced to set the gate.
- **I11 — Windows determinism unstated (low).** Generated-file byte-identity ("two syncs produce identical bytes") needs explicit LF-only + UTF-8-no-BOM writes, forward-slash path normalization, and `os.replace` atomic swaps, or idempotence checks flap between machines/editors.
- **I12 — Hygiene gaps (low).** `artifacts/` is not gitignored (plan assumes excluded); `.uv-cache/` at root and `backend/` is untracked-but-unignored; pre-commit exists only backend-scoped while governance checks are repo-wide.
...
- Constrain `validation` commands: must be non-destructive by policy; commands that require live-service env gates (`GOOGLE_LIVE_E2E`, `GOOGLE_LIVE_SEND_E2E`) are forbidden in annotations and may only run via an explicit human-invoked CLI flag.

### A3. MCP policy (§Initial policies → MCPs) — rewrite for the three-tier reality

- Tier definitions per §3.2. Manifest fields: tier (`deployment` / `user-connection` / `google-rest`), transport (SSE vs streamable HTTP is inferred from URL), probe policy, tool prefix, allowlist source (settings validator / per-connection / product registry), approval gating (deferred-tools), encryption at rest.
...
- Add explicit policies: (a) per-user connections are enumerated only from runtime evidence with provenance "database"; (b) the approval-gating asymmetry (MCP tools bypass deferred-tool approval) is recorded as an accepted-risk exception with an ADR, or scheduled for remediation — not silently assumed away; (c) frontend catalog entries must map to a backend `google_api_kind` or documented generic handling; (d) SSRF validation (`validate_webhook_url` on all MCP/OAuth URLs) is a protected invariant.
...
- OpenAPI extraction must run in an **isolated subprocess with a scrubbed environment** (no `.env` on the search path, `LOGFIRE_TOKEN` unset) because settings loading walks parent dirs and Logfire configures at import (I1). Reuse the pattern in `backend/tests/api/test_openapi.py`.

### A7. Configuration (§Initial policies → Configuration)

- **Never import `Settings` in governance tooling** — extract via AST (names, types, defaults, validators) and cross-check `.env.example` textually. (Direct consequence of I1.)
...
Restate dimension policy as a **five-point agreement check**: settings `EMBEDDING_DIMENSION` ↔ vectorstore guard/DDL ↔ migration CHECK ↔ persisted per-collection fingerprint ↔ cache-key version. Migration policy: "contiguous" = single-head linear revision graph; document the existing filename gaps as accepted so the policy doesn't fire on history.
...
- Phase 5 CI = a new job in the existing `ci.yml`. Until a git remote exists, "PR CI" gates run as `make governance-check` locally; **add establishing a remote (even private) to Phase 0** — both for enforcement substrate and because a single-disk governance history contradicts the plan's durability goals.
- Windows determinism (I11): all generated files written LF-only, UTF-8 without BOM, normalized forward-slash paths, `os.replace` atomic swaps; add a `.gitattributes` entry pinning generated governance files to LF.
...
- **Phase 0 pre-step (new):** land or stash the in-flight MCP→Google-REST work, commit, and establish a git remote. Take the baseline on a clean tree. (Resolves I4/I6.)
...
- **Phase 8 addition:** per-user MCP connection inventory (from DB, names/URLs/tool counts only — never tokens) compared against the frontend catalog and the approval-gating map.
...
8. **MCP tools bypass the approval dialog** (§3.2) — worth an explicit product/security decision, independent of governance tooling.
