...
|----------|---------|-------------|
| `PROJECT_NAME` | `fullstack` | Display name for the project |
...
| `TIMEZONE` | `EDT` | IANA timezone (e.g. `UTC`, `Europe/Warsaw`, `America/New_York`) |
...
| `GOOGLE_REDIRECT_URI` | `http://localhost:8100/api/v1/oauth/google/callback` | OAuth2 callback URL |
...
| `POSTGRES_DB` | `fullstack` | Database name |
...
| `LOGFIRE_SERVICE_NAME` | `fullstack` | Service name in Logfire dashboard |
...
| `S3_RAG_BUCKET` | `fullstack-rag` | Bucket name |
...
| `S3_BUCKET` | `fullstack` | Bucket name |
