...
- **CLI** — `uv run fullstack rag-sync` commands
...
uv run fullstack cmd rag-sync --connector my_source \
...
# Trigger a sync
curl -X POST http://localhost:8100/api/v1/rag/sync/sources/{source_id}/sync \
    -H "Authorization: Bearer $TOKEN"
```
